...
Mw      = 7.0 + 0.2*randn(100,1); % 100 sample would be good!
...
%    'regime', 'crustal');  
[L, W, stats] = mw2srcdim('Mw', Mw, 'fault', 'normal',...
    'regime', 'crustal');
...
xlabel('magnitude (Mw)'); ylabel('log_1_0(rupture width W)');
%axis([7 9 1.5 3]);
%axis([6 7.8 0.5 2]);
axis([6.2 7.8 0.75 2]);
%title('Prob. distribution around Mw 6.8 for crustal strike-slip fault')
title('Source-scaling relations for Mw 7.0 normal faulting earthquake')
...
xlabel('magnitude (Mw)'); ylabel('log_1_0(rupture length L)');
%axis([7 9 1.5 3]);
%axis([6 7.8 1 2.5]);
axis([6.2 7.8 1 2.5]);
